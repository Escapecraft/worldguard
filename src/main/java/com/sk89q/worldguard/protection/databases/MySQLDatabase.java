...
public class MySQLDatabase extends AbstractProtectionDatabase {
	...
	private void loadGlobal() {
		...
		try {
			...
			while (globalResultSet.next()) {
				...
				String claimType = globalResultSet.getString("claimtype");
				if (claimType != null) {
	...
	private void updateClaimType(ProtectedRegion region) throws SQLException {
		PreparedStatement clearCurrentClaimType = this.conn.prepareStatement(
				"DELETE FROM `claimtype` " +
						"WHERE `region_id` = ? " + 
						"AND `world_id = " + this.worldDbId
				);
		clearCurrentClaimType.setString(1, region.getId().toLowerCase());
		clearCurrentClaimType.execute();
		
		PreparedStatement insertClaimStatement = this.conn.prepareStatement(
		...
		insertClaimStatement.setString(1, region.getId().toLowerCase());
		insertClaimStatement.setString(2, region.getClaimType().toString().toLowerCase());
		
		insertClaimStatement.execute();
	}
}
